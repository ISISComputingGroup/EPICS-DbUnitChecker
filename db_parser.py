...
class Parser:
    ...
    def checkString(self, text):
        """
        Helper function to return the data between the next comma and closed bracket, even if it's a string containing commas/brackets
        """
        quote_pos = text.find('"')
    ...
    def getProps(self, keyword, text):
        """
    ...
    def parseDB(self):
        ...
        while rec_pos != -1:
            ...
            infos = self.getProps("info", field_text)

            #find fields
            fields = self.getProps("field", field_text)

            #populate records list
            rec = Record(self.dbfile.getDir(), rec_type.strip(), pv_name, infos, fields)
            recs.append(rec)
